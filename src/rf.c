...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
	     ...
	     int *outclts, int *labelts, double *proxts, double *errts,
             int *inbag, int *tmpFavorVec, int *tmpFavorLength) {
    /******************************************************************
	...
	*at, *a, *b, *mind, *nind, *jts, *oobpair;
    ...
    //favorLength = (int)*tmpFavorLength;
	favorLength = (int)(*tmpFavorLength);
	//warning("Length: %i", favorLength);
    ...
    favorVec =      (int *) S_alloc(favorLength, sizeof(int));
    if (oobprox) {
    	oobpair = (int *) S_alloc(near*near, sizeof(int));
    }

    //warning("%s",typeof(favorLength));
    //warning("%s",typeof(&favorLength));
    //warning("%d", favorLength);
    //warning("%d", &favorLength);

	for(j=0; j<favorLength; j++)
		favorVec[j] = tmpFavorVec[j];

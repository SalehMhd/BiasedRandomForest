...
c     SUBROUTINE BUILDTREE
     ...
     1     nclass, fLength, treemap, bestvar, bestsplit, bestsplitnext,
     1     tgini, nodestatus,nodepop, nodestart, classpop, tclasspop,
...
c     main program.
     ...
     1     iv(mred), nodeclass(nrnodes), mind(mred), fVec(fLength)
...
c      write(myString,'(I0)') fLength
c      write(myString,'(F10.2)') fLength
c      write(myString,'(I0)') fVec(12)
c      call rwarn(myString)
c      write(myString,'(i3)') fVec(5)
c      call rwarn(myString)
...
c     initialize for next call to findbestsplit
     ...
     1        best,ncase, jstat,mtry,win,wr,wl,mred,mind,fLength,fVec)
...
c     the coding into an integer of the categories going left.
     ...
     3     mred, mind, fLength, fVec)
      implicit double precision(a-h,o-z)
      integer fLength
      integer a(mdim,nsample), cl(nsample), cat(mdim),
     1     ncase(nsample), b(mdim,nsample), nn, j, fVec(fLength)
...
c     sampling mtry variables w/o replacement.
      do mt = 1, mtry
         call rrand(xrand)
         j = int(fLength * xrand) + 1
         j = fVec(j)
         if (mselected(j)) then
            j = int(nn * xrand) + 1
         end if

c         write(myString,'(I0)') j
c         call rwarn(myString)

...
"randomForest.default" <-
    function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             keep.inbag=FALSE, favorScale=NULL, ...) {
      #SALEH
      favor <- NULL
      for (e in 1:ncol(x)) {
        favor <- c(favor, rep(e,favorScale[e]))
      }
      #SALEH
    ...
    if (classRF) {
        ...
        rfout <- .C("classRF",
                    ...
                    favorVec = as.integer(favor),
                    favorLength = as.integer(length(favor)),
                    DUP=FALSE,
                    PACKAGE="BiasedRandomForest")[-1]
        if (keep.forest) {
